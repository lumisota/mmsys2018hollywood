...
VN_RUNS     = $(foreach PROTOCOL,$(PROTOCOLS),$(foreach RUN_NUMBER,$(RUN_NUMBERS),$(foreach PROFILE, $(PR_PROFILES), pr-$(PROTOCOL)-oo-$(PROFILE)-$(RUN_NUMBER))))
...
stage3/dash-cross-traffic: stage3/dash-cross-traffic.tar.gz
	@echo "================================================================================"
	@echo "== Extracting stage3/dash-cross-traffic.tar.gz"
	@tar -xvzf stage3/dash-cross-traffic.tar.gz -C stage3
	@echo "^^ Extracted stage3/dash-cross-traffic.tar.gz"
	@echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
...
stage3/data/sn-%.cclient-out stage3/data/sn-%.cserver-out stage3/data/sn-%.aclient-out stage3/data/sn-%.aserver-out: stage3/Vagrantfile-$(TCPH_KERNEL_REV)-$(TCPH_API_REV) stage3/scripts/dash-evaluation-linear-stablenetwork.py stage2/hollywood-$(TCPH_KERNEL_REV)-$(TCPH_API_REV)-libs.box | stage3/BBB_8bitrates_hd stage3/big_buck_bunny_1080p.y4m stage3/data 
	@echo "================================================================================"
	@echo "== Building $@"
	@echo "== `date`"
	mkdir -p stage3/sn-$*
...
stage4/data/sn_qos_stats.csv: $(foreach RUN,$(SN_LOSS_RUNS), stage3/data/$(RUN).aclient.out) $(foreach RUN,$(SN_DELAY_RUNS), stage3/data/$(RUN).aclient-out) stage4/extract_qos.R | stage4/data
	...
	awk 'BEGIN{OFS=":"}{print "Net",$0}' tmp2 > tmp
	sed 's/-tcp.*aclient-out//g' tmp > tmp2
	Rscript extract_qos.R tmp2 $@
	rm tmp tmp2

stage4/data/sn_rate_stats.csv: $(foreach RUN,$(SN_RUNS), stage3/data/$(RUN).aclient.out)| stage4/data
	grep BUFFER $(foreach RUN,$(SN_RUNS), stage3/data/$(RUN).aclient.out) | sed 's/^.*dash-1000-//g;s/-oo-/ /g;s/.aclient.*BUFFER://g;s/-/ /g' > $@
...
figures/results/stall_sn_%.pdf figures/results/startup_sn_%.pdf figures/results/ratedrops_sn_%.pdf figures/results/bitrate_sn_%.pdf: stage4/data/sn_rate_changes.csv stage4/data/sn_qos_stats.csv stage4/data/sn_rate_stats.csv stage5/generate_singlenet_qos_plots.R
	Rscript generate_singlenet_qos_plots.R

# ================================================================================================
# Stage 6: Other file generation
# ================================================================================================
