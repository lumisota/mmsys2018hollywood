...
\begin{algorithm}[t!]
	\begin{algorithmic}[1]
		...
		\State $p \gets \textit{Placeholder for effective buffer level calculation}$
		...
		\While {$\text{n} < \text{N}$}
			...
			\State ${q \gets GetBolaQualityEstimate(b_{now},  p)}$
...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\While {$\text{n} < \text{N}$}
			...
			\EndIf
			\State $p \gets \Delta t$
			\If {$L > 0 and p > ChunkDuration $}
				\State $p \gets p - ChunkDuration$
			\EndIf
			\State ${q \gets GetBolaQualityEstimate(b_{now}, p)}$		
